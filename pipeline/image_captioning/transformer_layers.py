...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        batch_size = q.size(0)
        num_dims = self.embed_dim // self.num_heads
        q = q.view(batch_size, -1, self.num_heads, num_dims).permute(0, 2, 1, 3).contiguous().view(batch_size * self.num_heads, -1, num_dims)
        k = k.view(batch_size, -1, self.num_heads, num_dims).permute(0, 2, 1, 3).contiguous().view(batch_size * self.num_heads, -1, num_dims)
        v = v.view(batch_size, -1, self.num_heads, num_dims).permute(0, 2, 1, 3).contiguous().view(batch_size * self.num_heads, -1, num_dims)

		# Scaled dot production
        scores = torch.bmm(q, k.transpose(-2, -1)) / math.sqrt(num_dims)
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(0).repeat(self.num_heads, 1, 1).unsqueeze(0).repeat(batch_size, 1, 1, 1)
            scores = scores.masked_fill(attn_mask.view(scores.size())==0, -float('inf'))
        attn_probs = self.dropout(F.softmax(scores, dim=-1))
        attn_out = torch.bmm(attn_probs, v)

        attn_out = attn_out.view(batch_size, self.num_heads, -1, num_dims).permute(0, 2, 1, 3).contiguous().view(batch_size, -1, self.embed_dim)
