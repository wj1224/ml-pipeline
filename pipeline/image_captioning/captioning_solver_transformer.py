...
class CaptioningSolverTransformer(object):
    ...
    def __init__(self, model, data, idx_to_word, **kwargs):
        ...
        self.tracking = kwargs.pop("tracking", False)
    ...
    def train(self, args):
        ...
        if self.tracking:
            mlflow.set_tracking_uri('http://192.168.0.54:31267')
            mlflow.start_run()
        ...
        for t in range(num_iterations):
            ...
            if self.verbose and t % self.print_every == 0:
                ...
                print(f'{datetime.now(timezone.utc).astimezone().isoformat()} loss={self.loss_history[-1]}')

                if self.tracking:
                  mlflow.log_metric("loss", float(self.loss_history[-1]), t)
        ...
        if self.tracking:
            mlflow.pytorch.log_model(
                ...
                artifact_path="image-captioning-transformers-model",
                registered_model_name="image-captioning-transformers-registered-model"
            )
            mlflow.end_run()
